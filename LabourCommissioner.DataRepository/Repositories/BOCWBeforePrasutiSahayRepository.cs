...
{
    public class BOCWBeforePrasutiSahayRepository : BaseRepository<TabModel>, IBOCWBeforePrasutiSahayRepository
    {
        public IConfiguration appConfig;
        public BOCWBeforePrasutiSahayRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<BOCWBPSYSchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    ...
                    var result = await conn.QueryFirstOrDefaultAsync<BOCWBPSYSchemeDetails>(Procedures.BOCW_PSY_GetApplicationSchemeDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<BOCWBPSYSchemeDetails> GetTotalsahayByServiceID(int serviceId)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    var result = (await conn.QueryAsync<BOCWBPSYSchemeDetails>(Procedures.GetTotalsahayByServiceID, queryParameters, commandType: CommandType.StoredProcedure)).FirstOrDefault();
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_psy_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWBPSYSchemeDetails schemeDetails)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_psy_insertschemedetails(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno,@in_expecteddeliverydate," +
                        "@in_prasutino,@in_age,@in_relation,@in_createdby,@in_isdeleted,@in_ipaddress,@in_hostname,@in_bankname,@in_bankbranch,@in_ifsccode,@in_bankaccountno,@in_accountholdername,@in_totalsahay,@in_schemaname,@in_tablename," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_psy_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_psy_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)    
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_psy_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
