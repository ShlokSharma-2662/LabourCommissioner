...
{
    public class BOCWHostelSahayYojanaRepository : BaseRepository<TabModel>, IBOCWHostelSahayYojanaRepository
    {
        public IConfiguration appConfig;
        public BOCWHostelSahayYojanaRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<BOCWHssySchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    BOCWHssySchemeDetails objGLWBhtySchemeDetails = new BOCWHssySchemeDetails();
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("in_applicationid", ApplicationId);
                    var result = (await conn.QueryAsync<BOCWHssySchemeDetails>(Procedures.bocw_hssy_getapplicationschemedetailsbyappid, queryParameters, commandType: System.Data.CommandType.StoredProcedure)).ToList();

                    //var result = await conn.QueryAsync<BOCWHssySchemeDetails>(Procedures.bocw_Hssy_getapplicationschemedetailsbyappid, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    if (result != null && result.Count() > 0)
                    {
                        ...
                        objGLWBhtySchemeDetails.lstfamilymemberDetails = result.Select(m => new familymember
                        {

                            famillymembernames = m.fg_famillymembernames,
                        }).ToList();

                    }

                    //Hostel  details Function call 

                    var results = (await conn.QueryAsync<BOCWHssySchemeDetails>(Procedures.bocw_hssy_getapplicationschemedetailsbyappidHostel, queryParameters, commandType: System.Data.CommandType.StoredProcedure)).ToList();

                    if (results != null && results.Count() > 0)
                    {
                        objGLWBhtySchemeDetails.lstStudentHostelMemberDetails = results.Select(m => new StudentHostelMemberDetails
                        {

                            sname = m.fg_sname,
                            isstayhostel = m.fg_isstayhostel,
                            hostelname = m.fg_hostelname,
                            hosteladdress = m.fg_hosteladdress,
                            hosteltype = m.fg_hosteltype,
                        }).ToList();

                    }

                    //Family details Function call 

                    var resultss = (await conn.QueryAsync<BOCWHssySchemeDetails>(Procedures.bocw_hssy_getapplicationschemedetailsbyappidfamilydetails, queryParameters, commandType: System.Data.CommandType.StoredProcedure)).ToList();

                    if (resultss != null && resultss.Count() > 0)
                    {


                        objGLWBhtySchemeDetails.lstStudentMemberDetails = resultss.Select(m => new StudentMemberDetails
                        {

                            name = m.fg_name,
                            age = m.fg_age,
                            stdorcourse = m.fg_stdorcourse,
                            schoolcollagename = m.fg_schoolcollagename,
                        }).ToList(); 
                    }
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_hssy_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWHssySchemeDetails schemeDetails, DataTable dtFamilyData, DataTable dtChildrenData, DataTable dtChildrenHostelData)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_hssy_insertschemedetails(@in_familytbl,@in_childrentbl,@in_childrenhosteltbl,@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_createdby," +
                        "@in_isdeleted,@in_ipaddress,@in_hostname,@in_familymemberno,@in_bankname,@in_bankbranch,@in_ifsccode,@in_bankaccountno,@in_accountholdername," +
                        "@in_schemaname,@in_tablename,@out_error,@out_msg,@out_id,@out_applicatinno)";
                    ResponseMessage res = new ResponseMessage();
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("@in_familytbl", new JsonParameter(JsonConvert.SerializeObject(dtFamilyData)));
                    queryParameters.Add("@in_childrentbl", new JsonParameter(JsonConvert.SerializeObject(dtChildrenData)));
                    queryParameters.Add("@in_childrenhosteltbl", new JsonParameter(JsonConvert.SerializeObject(dtChildrenHostelData)));
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_hssy_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
                ...
                using (var conn = GetConnection())
                {
                    ...
                    foreach (var item in lstdocumentFileDetails)
                    {
                        ...
                        {
                            var procName1 = "CALL public.bocw_hssy_tabentry(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno)";
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_hssy_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_hssy_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
