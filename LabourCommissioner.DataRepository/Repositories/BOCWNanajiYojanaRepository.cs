...
{
    public class BOCWNanajiYojanaRepository : BaseRepository<TabModel>, IBOCWNanajiYojanaRepository
    {
        public IConfiguration appConfig;
        public BOCWNanajiYojanaRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<BOCWNanjiSchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    BOCWNanjiSchemeDetails objGLWBhtySchemeDetails = new BOCWNanjiSchemeDetails();
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("in_applicationid", ApplicationId);

                    var result = (await conn.QueryAsync<BOCWNanjiSchemeDetails>(Procedures.BOCW_Nanji_GetApplicationSchemeDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure)).ToList();
                    if (result != null && result.Count() > 0)
                    {
                        //objGLWBhtySchemeDetails.familymemberno = result.Select(x => x.familymemberno).FirstOrDefault();
                        objGLWBhtySchemeDetails.schemeorg = result.Select(x => x.schemeorg).FirstOrDefault();
                        objGLWBhtySchemeDetails.housingscheme = result.Select(x => x.housingscheme).FirstOrDefault();
                        objGLWBhtySchemeDetails.houseowner = result.Select(x => x.houseowner).FirstOrDefault();
                        objGLWBhtySchemeDetails.housedetails = result.Select(x => x.housedetails).FirstOrDefault();
                        objGLWBhtySchemeDetails.otherhouse = result.Select(x => x.otherhouse).FirstOrDefault();
                        objGLWBhtySchemeDetails.otherhousedetails = result.Select(x => x.otherhousedetails).FirstOrDefault();
                        objGLWBhtySchemeDetails.houseloan = result.Select(x => x.houseloan).FirstOrDefault();
                        objGLWBhtySchemeDetails.loandetails = result.Select(x => x.loandetails).FirstOrDefault();
                        objGLWBhtySchemeDetails.housegetdate = result.Select(x => x.housegetdate).FirstOrDefault();
                        objGLWBhtySchemeDetails.districtapplydate = result.Select(x => x.districtapplydate).FirstOrDefault();
                        objGLWBhtySchemeDetails.otherschemuse = result.Select(x => x.otherschemuse).FirstOrDefault();
                        objGLWBhtySchemeDetails.otherschemename = result.Select(x => x.otherschemename).FirstOrDefault();
                        objGLWBhtySchemeDetails.money = result.Select(x => x.money).FirstOrDefault();
                        objGLWBhtySchemeDetails.instalments = result.Select(x => x.instalments).FirstOrDefault();
                        objGLWBhtySchemeDetails.totalsahay = result.Select(x => x.totalsahay).FirstOrDefault();
                        ...
                        objGLWBhtySchemeDetails.AccountHolderName = result.Select(x => x.AccountHolderName).FirstOrDefault();
                        objGLWBhtySchemeDetails.lstFamilyMemberDetails = result.Select(m => new NanajiFamilyMemberDetails
                        {
                            familydetailsid = m.fg_familydetailsid,
                            familymembersname = m.fg_familymembersname,
        ...
        public async Task<BOCWNanjiSchemeDetails> GetTotalsahayByServiceID(int serviceId)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    var result = (await conn.QueryAsync<BOCWNanjiSchemeDetails>(Procedures.GetTotalsahayByServiceID, queryParameters, commandType: CommandType.StoredProcedure)).FirstOrDefault();
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_nanji_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWNanjiSchemeDetails schemeDetails, DataTable dt)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL bocw_nanji_insertschemedetails(@in_documenttbl,@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_schemeorg,@in_housingscheme,@in_housegetdate,@in_districtapplydate,@in_houseowner," +
                        "@in_housedetails,@in_otherhouse,@in_otherhousedetails,@in_houseloan,@in_loandetails,@in_otherschemuse,@in_otherschemename," +
                        "@in_money,@in_instalments,@in_totalsahay,@in_familymembersname,@in_createdby," +
                    ...
                    //queryParameters.Add("@in_familymemberno", schemeDetails.familymemberno);
                    queryParameters.Add("@in_schemeorg", schemeDetails.schemeorg);
                    queryParameters.Add("@in_housingscheme", schemeDetails.housingscheme);
                    queryParameters.Add("@in_housegetdate", schemeDetails.housegetdate);
                    queryParameters.Add("@in_districtapplydate", schemeDetails.districtapplydate);
                    queryParameters.Add("@in_houseowner", schemeDetails.houseowner);
                    queryParameters.Add("@in_housedetails", schemeDetails.housedetails);
                    queryParameters.Add("@in_otherhouse", schemeDetails.otherhouse);
                    queryParameters.Add("@in_otherhousedetails", schemeDetails.otherhousedetails);
                    queryParameters.Add("@in_houseloan", schemeDetails.houseloan);
                    queryParameters.Add("@in_loandetails", schemeDetails.loandetails);
                    queryParameters.Add("@in_otherschemuse", schemeDetails.otherschemuse);
                    queryParameters.Add("@in_otherschemename", schemeDetails.otherschemename);
                    queryParameters.Add("@in_money", schemeDetails.money);
                    queryParameters.Add("@in_instalments", schemeDetails.instalments);
                    queryParameters.Add("@in_totalsahay", schemeDetails.totalsahay);
                    queryParameters.Add("@in_familymembersname", schemeDetails.familymembersname);
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_nanji_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_nanji_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_nanji_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
