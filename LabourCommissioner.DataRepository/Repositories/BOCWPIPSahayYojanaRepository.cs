...
namespace LabourCommissioner.DataRepository.Repositories
{
    ...
    public class BOCWPIPSahayYojanaRepository : BaseRepository<TabModel>, IBOCWPIPSahayYojanaRepository
    {
        public IConfiguration appConfig;
        public BOCWPIPSahayYojanaRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<BOCWPIPSchemeDetails> GetTotalsahayByServiceID(int serviceId)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    ...
                    var result = (await conn.QueryAsync<BOCWPIPSchemeDetails>(Procedures.GetTotalsahayByServiceID, queryParameters, commandType: CommandType.StoredProcedure)).FirstOrDefault();
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<BOCWPIPSchemeDetails> GettotalsahaybysahayId(int serviceid, int schemetype)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("_serviceid", serviceid);
                    queryParameters.Add("_relation", schemetype);
                    queryParameters.Add("_gender", 0);
                    queryParameters.Add("_other1", 0);
                    queryParameters.Add("_tharavid", 0);

                    var result = await conn.QueryFirstOrDefaultAsync<BOCWPIPSchemeDetails>(Procedures.GetTotalsahayByServiceID, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
           ...
           try
            {
                ...
                using (var conn = GetConnection())
               {
                    ...
                    var result = await conn.QueryFirstOrDefaultAsync<BOCWPIPSchemeDetails>(Procedures.BOCW_pip_GetApplicationSchemeDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_pip_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWPIPSchemeDetails schemeDetails) 
        {
            ...
            try
            {
                using (var conn = GetConnection()) 
                 {
                    var procName = "CALL public.bocw_pip_insertschemedetails(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_worktype,@in_schemetype,@in_joindate,@in_insuranceno,@in_insurancedetails, @in_totalsahay,@in_createdby," +
                        "@in_isdeleted,@in_ipaddress,@in_hostname,@in_bankname,@in_bankbranch,@in_ifsccode,@in_bankaccountno,@in_accountholdername," +
                        "@in_schemaname,@in_tablename,@in_premiumdate,@out_error,@out_msg,@out_id,@out_applicatinno)";
                    ResponseMessage res = new ResponseMessage();
                    var queryParameters = new DynamicParameters();
                    ...
                    queryParameters.Add("@in_worktype", schemeDetails.worktype);
                    queryParameters.Add("@in_schemetype", schemeDetails.schemetype);
                    queryParameters.Add("@in_joindate", schemeDetails.joindate);
                    queryParameters.Add("@in_premiumdate", schemeDetails.premiumdate);
                    queryParameters.Add("@in_insuranceno", schemeDetails.insuranceNo);
                    queryParameters.Add("@in_insurancedetails", schemeDetails.insurancedetails);
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_pip_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
                ...
                using (var conn = GetConnection())
                {
                    ...
                    foreach (var item in lstdocumentFileDetails)
                    {
                        ...
                        {
                            var procName1 = "CALL public.bocw_pip_tabentry(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno)";
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_pip_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_pip_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
