...
{
    public class BOCWTabletSahayYojanaRepository : BaseRepository<TabModel>, IBOCWTabletSahayYojanaRepository
    {
        public IConfiguration appConfig;
        public BOCWTabletSahayYojanaRepository(IConfiguration config) : base(config)
        {
           ...
           try
            {
                ...
                using (var conn = GetConnection())
               {
                    ...
                    var result = await conn.QueryFirstOrDefaultAsync<BOCWTSYSchemeDetails>(Procedures.BOCW_tsy_GetApplicationSchemeDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_tsy_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWTSYSchemeDetails schemeDetails) 
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_tsy_insertschemedetails(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_studydetails,@in_yearofs,@in_tabletno,@in_schoolorcollagename,@in_createdby," +
                    ...
                    queryParameters.Add("@in_studydetails", schemeDetails.studydetails);
                    queryParameters.Add("@in_yearofs", schemeDetails.Yearofs);
                    queryParameters.Add("@in_tabletno", schemeDetails.tabletno);
                    queryParameters.Add("@in_schoolorcollagename", schemeDetails.schoolorcollagename);
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_tsy_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
                ...
                using (var conn = GetConnection())
                {
                    ...
                    foreach (var item in lstdocumentFileDetails)
                    {
                        ...
                        {
                            var procName1 = "CALL public.bocw_tsy_tabentry(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno)";
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_tsy_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_tsy_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
