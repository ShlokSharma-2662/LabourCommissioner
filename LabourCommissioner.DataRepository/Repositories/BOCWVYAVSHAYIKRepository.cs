...
{
    public class BOCWVYAVSHAYIKRepository : BaseRepository<TabModel>, IBOCWVYAVSHAYIKRepository
    {
        public IConfiguration appConfig;
        public BOCWVYAVSHAYIKRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<BOCWVRSchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    BOCWVRSchemeDetails objGLWBhtySchemeDetails = new BOCWVRSchemeDetails();
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("in_applicationid", ApplicationId);

                    var result = await conn.QueryAsync<BOCWVRSchemeDetails>(Procedures.Bocw_vrgetapplicationschemedetailsbyappid, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    //var result = await conn.QueryFirstOrDefaultAsync<BOCWVRSchemeDetails>(Procedures.Bocw_vrgetapplicationschemedetailsbyappid, queryParameters, commandType: System.Data.CommandType.StoredProcedure);


                    if (result != null && result.Count() > 0)
                    {
                        objGLWBhtySchemeDetails.diseasestype = result.Select(x => x.diseasestype).FirstOrDefault();
                        objGLWBhtySchemeDetails.diseasename = result.Select(x => x.diseasename).FirstOrDefault();
                        objGLWBhtySchemeDetails.diseaseduration = result.Select(x => x.diseaseduration).FirstOrDefault();
                        objGLWBhtySchemeDetails.hospitalname = result.Select(x => x.hospitalname).FirstOrDefault();
                        objGLWBhtySchemeDetails.doctorname = result.Select(x => x.doctorname).FirstOrDefault();
                        objGLWBhtySchemeDetails.strength = result.Select(x => x.strength).FirstOrDefault();
                        objGLWBhtySchemeDetails.restduration = result.Select(x => x.restduration).FirstOrDefault();
                        objGLWBhtySchemeDetails.assistancepaid = result.Select(x => x.assistancepaid).FirstOrDefault();
                        objGLWBhtySchemeDetails.istakeanyotherassistance = result.Select(x => x.istakeanyotherassistance).FirstOrDefault();
                    ...
                    var result1 = await conn.QueryAsync<BOCWVRSchemeDetails>(Procedures.Bocw_vrgetapplicationschemedetailsbyappidotherschemedetails, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    if (result1 != null && result1.Count() > 0)
                    {
                        objGLWBhtySchemeDetails.lstOtherSchemeDetails = result1.Select(m => new BOCWVR_OtherSchemeDetails
                        {
                            otherschemename = m.fg_otherschemename,
                            otherschemesahay = m.fg_otherschemesahay,
                            restschemename = m.fg_restschemename,
                            restschemesahay = m.fg_restschemesahay
                        }).ToList();
        ...
        public async Task<IEnumerable<SelectListItem>> GetDieases(long type)
        {
            try
            {
                ...
                {
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("_type", type);
                    var result = (await conn.QueryAsync<SelectListItem>(Procedures.binddiseasesandinjuriesmaster, queryParameters, commandType: CommandType.StoredProcedure));
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_vr_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(BOCWVRSchemeDetails schemeDetails, DataTable dtOtherSchemeData)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_vr_insertschemedetails(@in_otherschemedetailstbl,@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_diseasename,@in_diseaseduration,@in_hospitalname,@in_doctorname,@in_strength,@in_restduration,@in_assistancepaid,@in_istakeanyotherassistance,@in_createdby," +
                        "@in_isdeleted,@in_ipaddress,@in_hostname,@in_bankname,@in_bankbranch,@in_ifsccode,@in_bankaccountno,@in_accountholdername,@in_type," +
                        "@in_totalsahay,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_id,@out_applicatinno)";
                    ResponseMessage res = new ResponseMessage();
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("@in_otherschemedetailstbl", new JsonParameter(JsonConvert.SerializeObject(dtOtherSchemeData)));
                    ...
                    queryParameters.Add("@in_diseasename", schemeDetails.diseasename);
                    queryParameters.Add("@in_diseaseduration", schemeDetails.diseaseduration);
                    queryParameters.Add("@in_hospitalname", schemeDetails.hospitalname);
                    queryParameters.Add("@in_doctorname", schemeDetails.doctorname);
                    queryParameters.Add("@in_strength", schemeDetails.strength);
                    queryParameters.Add("@in_restduration", schemeDetails.restduration);
                    queryParameters.Add("@in_assistancepaid", schemeDetails.assistancepaid);
                    queryParameters.Add("@in_istakeanyotherassistance", schemeDetails.istakeanyotherassistance);
                    ...
                    queryParameters.Add("@in_type", schemeDetails.diseasestype);
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.bocw_vr_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_vr_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.bocw_vr_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
