...
{
    public class GLWBHigherStudyRepository : BaseRepository<TabModel>, IGLWBHigherStudyRepository
    {
        public IConfiguration appConfig;
        public GLWBHigherStudyRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<GLWBHSS_PersonalDetails> GetPersonalDetailsByRegNo(string RegistrationNo)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    ...
                    var result = await conn.QueryFirstOrDefaultAsync<GLWBHSS_PersonalDetails>(Procedures.GetPersonalDetailsByRegNo, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<GLWBHSS_PersonalDetails> GetApplicationDetailsByAppId(long ApplicationId, string schemaname, string tablename)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    ...
                    var result = await conn.QueryFirstOrDefaultAsync<GLWBHSS_PersonalDetails>(Procedures.GLWB_GetApplicationDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<GLWBHSS_SchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("in_applicationid", ApplicationId);
                    var result = await conn.QueryFirstOrDefaultAsync<GLWBHSS_SchemeDetails>(Procedures.GLWBHSS_GetApplicationSchemeDetailsByAppId, queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    return result;
                }
            }
            catch (Exception ex)
        ...
        public async Task<ResponseMessage> AddUpdateApplication(GLWBHSS_PersonalDetails personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hss_insertpersonaldetails(@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(GLWBHSS_SchemeDetails schemeDetails)
        {
            ...
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hss_insertschemedetails(@in_applicationid,@in_serviceid,@in_userid,@in_sequenceno," +
                        "@in_courseid,@in_percentage,@in_year,@in_enirmancardno,@in_createdby," +
                        "@in_isdeleted,@in_ipaddress,@in_hostname,@in_bankname,@in_bankbranch," +
                        "@in_ifsccode,@in_bankaccountno,@in_schemaname,@in_tablename," +
                        "@in_examyear12th,@in_exammonth12th,@in_examyearentrance,@in_exammonthentrance,@in_totalsahay," +
                        "@out_error,@out_msg,@out_id,@out_applicatinno)";
                    ResponseMessage res = new ResponseMessage();
                    var queryParameters = new DynamicParameters();
                    ...
                    queryParameters.Add("@in_year", schemeDetails.ExamYear12th);
                    ...
                    queryParameters.Add("@in_examyear12th", schemeDetails.ExamYear12th);
                    queryParameters.Add("@in_exammonth12th", schemeDetails.ExamMonth12th);
                    queryParameters.Add("@in_examyearentrance", schemeDetails.ExamYearEntrance);
                    queryParameters.Add("@in_exammonthentrance", schemeDetails.ExamMonthEntrance);
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetails(IList<DocumentFileDetails> lstdocumentFileDetails)
        {
            try
            {
                var procName = "CALL public.glwb_hss_insertdocumentdetails(@in_applicationid,@in_documentmasterid,@in_documentname,@in_documentnumber,@in_createddate,@in_CreatedBy," +
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hss_addupdateappdeclaration(@in_applicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hss_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
                    ...
                    queryParameters.Add("@in_schemaname", nameof(EnumLookup.schemaname.glwb_hss));
        ...
        public async Task<GLWBHSS_SchemeDetails> GetTotalsahayByServiceID(int serviceId, int course)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("_serviceid", serviceId);
                    queryParameters.Add("_relation", course); // passing course in relation parameter
                    queryParameters.Add("_gender", 0); 
                    queryParameters.Add("_other1", 0);
                    queryParameters.Add("_tharavid", 0);
                    var result = (await conn.QueryAsync<GLWBHSS_SchemeDetails>(Procedures.GetTotalsahayByServiceID, queryParameters, commandType: CommandType.StoredProcedure)).FirstOrDefault();
                    return result;
                }
            }
            catch (Exception ex)
