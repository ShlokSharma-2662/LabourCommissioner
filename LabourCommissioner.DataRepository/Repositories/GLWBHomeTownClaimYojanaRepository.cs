...
{
    public class GLWBHomeTownClaimYojanaRepository : BaseRepository<TabModel>, IGLWBHomeTownClaimYojanaRepository
    {
        public IConfiguration appConfig;
        public GLWBHomeTownClaimYojanaRepository(IConfiguration config) : base(config)
        {
        ...
        public async Task<GLWBhtySchemeDetails> GetApplicationSchemeDetailsByAppId(long ApplicationId)
        {
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    ...
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("in_applicationid", ApplicationId);
                    var result = (await conn.QueryAsync<GLWBhtySchemeDetails>(Procedures.glwb_hty_claim_getapplicationschemedetailsbyappid, queryParameters, commandType: System.Data.CommandType.StoredProcedure)).ToList();
                    if (result != null && result.Count() > 0)
                    {
                        ...
                        objGLWBhtySchemeDetails.lstFamilyMemberTravelDetails = result.Select(m => new FamilyMemberTravelDetails
                        {
                            travellingdetailsid = m.fg_travellingdetailsid,
                            travelledby = m.fg_travelledby,
                            travelstatus = m.fg_travelstatus,
                            travelldate = m.fg_travelldate,
                            fromplace = m.fg_fromplace,
                            toplace = m.fg_toplace,
                            ticketno = m.fg_ticketno,
                            ticketamount = m.fg_ticketamount
                        }).ToList();
        ...
        public async Task<ResponseMessage> AddUpdateApplication(GLWBhty_PersonalDetailsModel personalDetailsModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hty_claim_insertpersonaldetails(@in_applicationid,@in_registrationid,@in_userprofileid,@in_photo,@in_name,@in_nameingujarati," +
        ...
        public async Task<ResponseMessage> AddSchemeDetails(GLWBhtySchemeDetails schemeDetails, DataTable dt)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hty_claim_insertschemedetails(@in_documenttbl,@in_applicationid,@in_claimapplicationid,@in_serviceid,@in_userid,@in_totalsahay,@in_sequenceno," +
                        "@in_schemename,@in_createdby," +
        ...
        public async Task<ResponseMessage> FinalSubmit(FinalSubmitModel finalSubmitModel)
        {
            try
            {
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hty_claim_addupdateappdeclaration(@in_applicationid,@in_claimapplicationid,@in_registrationid,@in_userid,@in_isagree,@in_ipaddress,@in_hostname," +
        ...
        public async Task<ResponseMessage> AddUpdateDocumentDetailsNew(DataTable dtData)
        {
            ...
            try
            {
                ...
                using (var conn = GetConnection())
                {
                    var procName = "CALL public.glwb_hty_claim_insertdocumentdetails_json(@in_documenttbl,@in_isupdated,@in_schemaname,@in_tablename,@out_error,@out_msg,@out_applicationid,@out_id)";
