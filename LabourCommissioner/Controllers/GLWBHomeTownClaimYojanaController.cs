...
{
    public class GLWBHomeTownClaimYojanaController : Controller
    {
        ...
        private readonly IGLWBHomeTownClaimYojanaService _iGLWBHomeTownYojanaservice;

        public GLWBHomeTownClaimYojanaController(IGLWBHomeTownClaimYojanaService iGLWBHomeTownYojanaService, IConfiguration config, IHttpClientFactory clientFactory, IHttpContextAccessor httpContextAccessor, IWebHostEnvironment webHostEnvironment)
        {
        ...
        public IActionResult AppPersonalDetails(string strid, string strTabId, string strClaimApplicationId, string strApplicationId, string strOrgServiceId = "")
        {
            ...
            //if (id == 16)
            //{
            var model = _iGLWBHomeTownYojanaservice.GetTabSequenceByApplicationId(ClaimApplicationId, OrgServiceId, nameof(EnumLookup.schemaname.glwb_hty_claim), nameof(EnumLookup.tablename.tabentry));
        ...
        [HttpGet]
        public async Task<IActionResult> PersonalDetails(string ServiceId, string strApplicationId, bool isFilled, string strClaimApplicationId)
        {
            ...
            return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaPersonalDetails", model1);
            //}


            //return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaPersonalDetails", model);
        ...
        [HttpGet]
        public async Task<IActionResult> GLWBHomeTownYojanaClaimSchemeDetails(string ServiceId, string strApplicationId, bool isFilled, string strClaimApplicationId)
        {
            ...
            ViewBag.ApplicationId = ApplicationId;
            ViewBag.ClaimApplicationId = Convert.ToInt64(strClaimApplicationId);
            ...
            return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaClaimSchemeDetails", model1);
        }

        [HttpGet]
        public async Task<IActionResult> GLWBHomeTownYojanaDocument(string ServiceId, string strApplicationId, bool isFilled, string strClaimApplicationId)
        {
            ...
            if (isFilled == true)
            {
                var GetUploadedDocuments = await _iGLWBHomeTownYojanaservice.GetUploadedDocuments(Convert.ToInt64(ApplicationId), Convert.ToInt32(ServiceId), nameof(EnumLookup.schemaname.glwb_hty_claim), nameof(EnumLookup.tablename.documentdetails));
                return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaDocument", GetUploadedDocuments);
            }
            else
            {
                return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaDocument", model);
            }
        }
        ...
        public IActionResult TermsCondition(string ServiceId, string strApplicationId, bool isFilled, string strClaimApplicationId)
        {
            ...
            return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaTermsCondition");
        }
        ...
        public async Task<IActionResult> AddUpdateApplication(GLWBhty_PersonalDetailsModel personalDetails, string strTabId, string strApplicationNo)
        {
            ...
            personalDetails.schemaname = nameof(EnumLookup.schemaname.glwb_hty_claim);
        ...
        public async Task<IActionResult> AddSchemeDetails(GLWBhtySchemeDetails schemeDetails, int TabSequenceNo, int ApplicationId)
        {
            ...
            if (schemeDetails.lstFamilyMemberTravelDetails != null && schemeDetails.lstFamilyMemberTravelDetails.Count() > 0)
            {
                var lstFamilyMemberTravelDetails = new List<FamilyMemberTravelDetails>();
                var RegistrationId = Convert.ToInt32(_claimPincipal.FindFirstValue("RegistrationId"));

                foreach (var item in schemeDetails.lstFamilyMemberTravelDetails)
                {

                    var objFamilyMemberDetails = new FamilyMemberTravelDetails();
                    //var couchDBRequest = new CouchDBRequest();
                    //var couchDBResponse = new CouchDBResponse();
                    //if (item.Ticket != null && item.Ticket.FileName != "" && item.Ticket.Length > 0)
                    //{
                    //    var extension = Path.GetExtension(item.Ticket.FileName);
                    //    string fileWithOutExt = item.Ticket.FileName.Substring(0, item.Ticket.FileName.Length - extension.Length);
                    ...
                    //    using (var msstream = new MemoryStream())
                    //    {
                    //        await item.Ticket.CopyToAsync(msstream);
                    //        fileBytes = msstream.ToArray();
                    //    }

                    //    couchDBRequest.FileName = FileName;//item.IdImage.Ticket.FileName;
                    //    couchDBRequest.AttachmentData = fileBytes;
                    //    couchDBRequest.FileExtension =
                    //        Path.GetExtension(item.Ticket.FileName).Replace(".", "").ToUpper();
                    ...
                    objFamilyMemberDetails.travellingdetailsid = item.travellingdetailsid;
                    objFamilyMemberDetails.travelledby = item.travelledby;
                    objFamilyMemberDetails.travelstatus = item.travelstatus;
                    objFamilyMemberDetails.toplace = item.toplace;
                    objFamilyMemberDetails.fromplace = item.fromplace;
                    objFamilyMemberDetails.travelldate = item.travelldate;
                    objFamilyMemberDetails.ticketno = item.ticketno;
                    objFamilyMemberDetails.ticketamount = item.ticketamount;
                    ...
                    lstFamilyMemberTravelDetails.Add(objFamilyMemberDetails);
                ...
                var dtData = CommonUtils.ToDataTable(lstFamilyMemberTravelDetails);
            ...
            return RedirectToAction("AppPersonalDetails", "GLWBHomeTownClaimYojana", schemeDetails);

        }

        public async Task<IActionResult> UploadDocument(IList<DocumentFileDetails> documentFileDetails, string strTabSequenceNo, string strApplicationId, string strServiceId, string strClaimApplicationId)
        {
            //if(ModelState.Is)
            ...
            if (documentFileDetails != null && documentFileDetails.Count() > 0)
            {
                ...
                foreach (var item in documentFileDetails)
                {
                        ...
                        item.IdImage.File.Length > 0)
                    {
                        ...
                        {
                            return PartialView("GLWBHomeTownClaimYojana/_GLWBHomeTownYojanaDocument");
                        }
                    }
                }
            ...
            //return View();
            return RedirectToAction("AppPersonalDetails", "GLWBHomeTownClaimYojana", documentFileDetails);
        }
        ...
        public async Task<IActionResult> FinalSubmit(FinalSubmitModel finalSubmitModel, string strApplicationId, string strServiceId, string strTabSequenceNo, string strClaimApplicationId)
        {
            ...
            finalSubmitModel.schemaname = nameof(EnumLookup.schemaname.glwb_hty_claim);
