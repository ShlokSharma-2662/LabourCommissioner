...
namespace LabourCommissioner.Controllers
{
    public class ServiceRoutineController : Controller
    {
        ...
        private readonly IServiceRoutineService _iServiceRoutineService;
        ...
        public ServiceRoutineController(IWebHostEnvironment webHostEnvironment, IServiceRoutineService iServiceRoutineService, IConfiguration config, IHttpClientFactory clientFactory,
            IHttpContextAccessor httpContextAccessor)
        {
            _webHostEnvironment = webHostEnvironment;
            _iServiceRoutineService = iServiceRoutineService ?? throw new ArgumentNullException(nameof(_iServiceRoutineService));
        ...
        public async Task BOCWUploadFileOnServer(string fromDate = "", string toDate = "")
        {
            ...
            modelList = await _iServiceRoutineService.BOCWGetAadeshDataForRoutine(dtFromDate, dtToDate);
            ...
            if (modelList != null && modelList.Count() > 0)
            {
                ...
                if (fileuploadresult == 1)
                {
                    ...
                    updatemodelList = await _iServiceRoutineService.UpdateBOCWPaymentInfo(payinfoids.Trim(','), fileName, (int)EnumLookup.AadeshPaymentInfoStatus.Approve, (int)EnumLookup.AadeshPaymentInfoStatus.Pending);
        ...
        public async Task BOCWFetchReturnCSVFileFromServer()
        {
            ...
            try
            {
                ...
                modelList = await _iServiceRoutineService.BOCWGetAadeshDataForFetchReturnCSVFile();
                ...
                if (finalResult != null && finalResult.Count() > 0)
                {
                    ...
                    foreach (var item in fileToRead)
                    {
                        ...
                        if (dtData != null && dtData.Rows.Count > 0)
                        {
                            ...
                            responseMessage = await _iServiceRoutineService.SaveBOCWPaymentResponse(dtData, CommonUtils.GetLocalIPAddress(), CommonUtils.GetHostName());
                        }
        ...
        public async Task GLWBUploadFileOnServer(string fromDate = "", string toDate = "")
        {
            ...
            modelList = await _iServiceRoutineService.GLWBGetAadeshDataForRoutine(dtFromDate, dtToDate);
            var finalResult = modelList.ToList();
            if (modelList != null && modelList.Count() > 0)
            {
                ...
                if (fileuploadresult == 1)
                {
                    ...
                    updatemodelList = await _iServiceRoutineService.UpdateGLWBPaymentInfo(payinfoids.Trim(','), fileName, (int)EnumLookup.AadeshPaymentInfoStatus.Approve, (int)EnumLookup.AadeshPaymentInfoStatus.Pending);
        ...
        public async Task GLWBFetchReturnCSVFileFromServer()
        {
            ...
            try
            {
                ...
                modelList = await _iServiceRoutineService.GLWBGetAadeshDataForFetchReturnCSVFile();
                var finalResult = modelList.DistinctBy(d => d.filename).ToList();
                ...
                if (finalResult != null && finalResult.Count() > 0)
                {
                    ...
                    foreach (var item in fileToRead)
                    {
                        ...
                        if (dtData != null && dtData.Rows.Count > 0)
                        {
                            ...
                            ResponseMessage responseMessage = new ResponseMessage();
                            responseMessage = await _iServiceRoutineService.SaveGLWBPaymentResponse(dtData, CommonUtils.GetLocalIPAddress(), CommonUtils.GetHostName());
                        }
